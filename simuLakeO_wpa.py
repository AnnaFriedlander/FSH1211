...
Lake O (codes 1-male; 2-female : 20 fish), and
...
1000 replications of the following simulation (each with a new random seed) are run:
  ...
  * Waipa fish population (1000 fish) undergo *mating scheme* for 100 gens/years
  MIGRATION:
  * Pre-simulation, 20 random Waipa fish removed and 20 Lake O fish migrated 
...
def simuVanilla(replications=1000,gens=100,rsize=20,wsize=50,maxAge=5,matingAge=3):
    ...
    #create Waipa and Lake O populations
    ...
    for i in range(popsize):
        ...
        if(i<rsize):
           Lake_O.individual(i).setGenotype(geno)
           Lake_O.individual(i).setSex(sex)   
        else:
    ...
    #"lakeowai" (1000 waipa fish to receive 20 Lake O fish)
    ...
    fileout.write('preWaipa-preWaipa2\tpreWaipa-preLakeowai\tpreWaipa2-preLakeowai\t')
    fileout.write('postWaipa-postWaipa2\tpostWaipa-postLakeowai\tpostWaipa2-postLakeowai\t')
    fileout.write('preWaipa-postWaipa\tpreWaipa-postWaipa2\tpreWaipa-postLakeowai\t')
    fileout.write('preWaipa2-postWaipa\tpreWaipa2-postWaipa2\tpreWaipa2-postLakeowai\t')
    fileout.write('preLakeowai-postWaipa\tpreLakeowai-postWaipa2\tpreLakeowai-postLakeowai\n')
    ...
    for i in range(replications):
        ...
        #remove 20 random Waipa fish from "lakeowai"
        pop = pops.clone()
        while (pop.subPopSize(subPop=2) > 980): 
            pop.removeIndividuals(random.randint(2000,3000))

        #add 20 Lake_O fish to "lakeowai"
        pop.mergeSubPops(subPops=[2,3])
        pop.setSubPopName('Waipa',0)
        pop.setSubPopName('Waipa2',1)
        pop.setSubPopName('Lakeowai',2)

        #print pre-drift lakeowai population to FSTAT file
        rwai = pop.extractSubPops(subPops=[2])
        #rwai.setSubPopName('Lakeowai',0)
        fname = "LAKEOWAI-REP_%s.gen" % (i+1)
        gpout(fname, rwai)

        #calculate pair-wise Fst on pre-evolution Waipa, Waipa2, Lakeowai
        ...
        fname = 'LAKEO_WPA-REP_%s.gen' % (i+1)
