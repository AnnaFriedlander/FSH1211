#!/usr/bin/env python
#
# Simulate introduction of Lake O fish into Waipa population
...
Rotorua (codes 1-male; 2-female : 20 fish),
Lake O (codes 3-male; 4-female : 20 fish), and
Waipa (codes 5-male; 6-female : 50 fish) 
...
1000 replications of the following simulation (each with a new random seed) are run:
  ...
  * Waipa fish population (1000 fish) undergo *mating scheme* for 100 gens/years
  MIGRATION:
  * Pre-simulation, 20 random Waipa fish removed and 20 random Rotorua and 
    Lake O fish (with 10 from each locale; each sex balanced 5 male/5 female)
    migrated to Waipa pop (total post-migration pop = 1000)
...
def simuVanilla(replications=1000,gens=100,rsize=20,wsize=50,maxAge=5,matingAge=3):
    ...
    #create Rotorua, Lake O, and Waipa populations
    Rotorua = Population(size=[rsize], loci=numloci, infoFields=['age'],
                         lociNames=locinam, subPopNames=['Rotorua'])
    Lake_O  = Population(size=[rsize], loci=numloci, infoFields=['age'],
                         lociNames=locinam, subPopNames=['Lake_O'])
    ...
    for i in range(popsize):
        ...
        if(i<rsize):
           ...
           Lake_O.individual(i-rsize).setGenotype(geno) 
           Lake_O.individual(i-rsize).setSex(sex)   
        else:
           Waipa.individual(i-(rsize*2)).setGenotype(geno) 
           Waipa.individual(i-(rsize*2)).setSex(sex) 
    ...
    Lake_O.setIndInfo([random.randint(0, maxAge) for x in range(popsize)], 'age')
    ...
    Lake_O.setVirtualSplitter(InfoSplitter('age', cutoff=[matingAge, maxAge+0.1]))
    ...
    #"rl_wpa" (1000 waipa fish to receive 20 Roto/Lake O fish)
    pops = simu.population(0).clone()
    pops.addIndFrom(simu.population(0))
    pops.addIndFrom(simu.population(0))
    pops.addIndFrom(Rotorua)
    pops.addIndFrom(Lake_O)

    #open file and write headers
    fileout = open('simuOUT', 'w')
    fileout.write('preWaipa-preWaipa2\tpreWaipa-preRl_wpa\tpreWaipa2-preRl_wpa\t')
    fileout.write('postWaipa-postWaipa2\tpostWaipa-postRl_wpa\tpostWaipa2-postRl_wpa\t')
    fileout.write('preWaipa-postWaipa\tpreWaipa-postWaipa2\tpreWaipa-postRl_wpa\t')
    fileout.write('preWaipa2-postWaipa\tpreWaipa2-postWaipa2\tpreWaipa2-postRl_wpa\t')
    fileout.write('preRl_wpa-postWaipa\tpreRl_wpa-postWaipa2\tpreRl_wpa-postRl_wpa\n')
    ...
    for i in range(replications):
        ...
        #remove 20 random Waipa fish from "rl_wpa"
        pop = pops.clone()
        while (pop.subPopSize(subPop=2) > 980): 
            pop.removeIndividuals(indexes=random.randint(2000,3000))

        #remove random 5 male and 5 female fish from Rotorua; 
        #remove random 5 male and 5 female fish from Lake O
        removed=0;roto=0;lakeo=0;rotoM=0;rotoF=0;lakeoM=0;lakeoF=0;
        index0 = pop.subPopSize(subPop=0) + pop.subPopSize(subPop=1) + pop.subPopSize(subPop=2)
        while removed < 20 :
            #select random individuals from Rotorua until 5 males and 5 females removed
            while roto < 10:
                index1 = index0 + pop.subPopSize(subPop=3) - 1
                num = random.randint(index0,index1)
                sex = pop.individual(num).sex()
                if(sex==1 and rotoM < 5):
                    pop.removeIndividuals(indexes=num)
                    rotoM += 1
                    roto +=1
                    removed +=1
                elif(sex==2 and rotoF < 5):
                    pop.removeIndividuals(indexes=num)
                    rotoF += 1
                    roto +=1
                    removed +=1
            #select random individuals from Lake O until 5 males and 5 females removed
            while lakeo < 10:
                index1 = index0 + pop.subPopSize(subPop=3)
                index2 = pop.popSize() - 1
                num = random.randint(index1,index2)
                sex = pop.individual(num).sex()
                if(sex==1 and lakeoM < 5):
                    pop.removeIndividuals(indexes=num)
                    lakeoM += 1
                    lakeo +=1
                    removed +=1
                elif(sex==2 and lakeoF < 5):
                    pop.removeIndividuals(indexes=num)
                    lakeoF += 1
                    lakeo +=1
                    removed +=1
        ...
        #add 20 Rotorua and Lake O fish to "rl_wpa"
        pop.mergeSubPops(subPops=[2,3,4])
        pop.setSubPopName('Waipa',0)
        pop.setSubPopName('Waipa2',1)
        pop.setSubPopName('Rl_wpa',2)


        #print pre-drift rl_wpa population to FSTAT file
        rwai = pop.extractSubPops(subPops=[2])
        fname = "predriftRLWPA-REP_%s.gen" % (i+1)
        gpout(fname, rwai)

        #calculate pair-wise Fst on pre-evolution Waipa, Waipa2, rl_wpa
        ...
        fname = 'postdriftpops-REP_%s.gen' % (i+1)
